...
export function loginAdmin(email,password){const u=ADMIN_USERS.find(x=>x.email===email&&x.password===password);if(!u)throw new Error('Invalid Admin/HR email or password');setSession({type:u.role,email:u.email,name:u.name});return u;}
